...
def start_conversation(tg_client_id: int) -> Tuple[int, int]:
    ...
    :return: If the given user is in a conversation already, `(-1, -1)` is returned. If the given user is in
        conversation(s) as an operator (an operator who is currently a client), `(-2, -2)` is returned. If there are no
        operators available for the user (for example, they all are busy), `(-3, -3)` is returned. Otherwise a tuple
        where first element is the <b>telegram</b> operator id and the second is the <b>local</b> client id
    """
    with PrettyCursor() as cursor:
        try:
            ...
            except TypeError:  # No operators available
                return -3, -3
